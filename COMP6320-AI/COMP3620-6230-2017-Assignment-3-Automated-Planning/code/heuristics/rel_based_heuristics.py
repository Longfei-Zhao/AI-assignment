...
class hff(_RelaxationHeuristic):
    ...
    def calc_goal_h(self):
        ...
        res = set()
        unvis = set(self.goals)


        while unvis:
            fact = unvis.pop()
            if self.facts[fact].level == float('inf'):
                return float('inf')
            if self.facts[fact].cheapest_achiever:
                res.add(self.facts[fact].cheapest_achiever)
                unvis.union(self.facts[fact].cheapest_achiever.preconditions)
...
class hadm(_RelaxationHeuristic):
    ...
    def __call__(self, node):
        ...
        for fact in self.facts.values():
            if fact.name in node.state:
                h[fact.name] = 0
            else:
                h[fact.name] = float('inf')
        flag = True
        while flag:
            flag = False
            for action in self.actions:
                for preconditon in action.preconditions:
                    if h[preconditon] > max([h[preconditon] for preconditon in action.preconditions]) + 1:
                        flag = True
                        h[preconditon] = max([h[preconditon] for preconditon in action.preconditions]) + 1
        return float(max([h[g] for g in self.goals]))
