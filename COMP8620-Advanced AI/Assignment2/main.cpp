...
#include "grid.hpp"
// Streams for logging
std::ofstream logg;        // A verbose human-readable log
...
void mainLoop(Agent &ai, Environment &env, options_t &options) {
	...
	for (unsigned int cycle = 1; !env.isFinished(); cycle++) {
		...
		if (terminate_check && ai.lifetime() > terminate_lifetime) {
			logg << "info: terminating lifetiment" << std::endl;
			break;
		}
		...
		// Log this turn
		logg << "cycle: " << cycle << std::endl;
		logg << "observation: " << observation << std::endl;
		logg << "reward: " << reward << std::endl;
		logg << "action: " << action << std::endl;
		logg << "explored: " << (explored ? "yes" : "no") << std::endl;
		logg << "explore rate: " << explore_rate << std::endl;
		logg << "total reward: " << ai.reward() << std::endl;
		logg << "average reward: " << ai.averageReward() << std::endl;
...
int main(int argc, char *argv[]) {
	...
	else if (environment_name.find("1d") != std::string::npos) {
		env = new Maze1d(options);
		options["agent-actions"] = "2";
		options["observation-bits"] = "1";
		options["reward-bits"] = "1";
	
	}
	else if (environment_name.find("chee") != std::string::npos ) {
	...
	else if (environment_name.find("4x4")!= std::string::npos) {
		env = new Grid(options);
		options["agent-actions"] = "4";
		options["observation-bits"] = "1";
		options["reward-bits"] = "1";
